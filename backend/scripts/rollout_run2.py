"""
CONTROLLED PRODUCTION ROLLOUT - Run 2
Batch size: 5 scholarships
"""
import json
import logging
import sqlite3
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('rollout_run2.log', mode='w'),
    ]
)
...
def main():
    print("=" * 70)
    print("CONTROLLED PRODUCTION ROLLOUT - RUN 2")
    print("=" * 70)
    ...
    # Get IDs that will be selected (next 5 due candidates, skipping already processed)
    ...
    if not selected_ids:
        print("\nNo unverified scholarships due. Run 2 skipped.")
        return True

    # Start scheduler with batch_size=5
    print(f"\n{'='*70}")
    print(f"STARTING SCHEDULER (batch_size=5)")
    print(f"{'='*70}")

    config = SchedulerConfig(batch_size=5, max_workers=2)
